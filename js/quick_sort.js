...
function quick_partition (start, end)
{
    ...
    div_update(divs[start],div_sizes[start],"yellow");//Color update
        ...
        for(var j =start + 1; j <= end ; j++ )
        {
            ...
            if (div_sizes[ j ] < piv)
            {
                ...
                val_update(divs[j],div_sizes[j],divs[j].innerHTML,"red");//Height update
    ...
    var temp=divs[start].innerHTML;
    divs[start].innerHTML=divs[i-1].innerHTML;
    divs[i-1].innerHTML=temp;

    val_update(divs[start],div_sizes[start],divs[start].innerHTML,"red");//Height update
    val_update(divs[i-1],div_sizes[i-1],divs[i-1].innerHTML,"red");//Height update
